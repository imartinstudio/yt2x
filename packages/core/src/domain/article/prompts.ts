...
- Markdown 列表必须规范：有序列表使用 \`1. 2. 3.\`，无序列表使用 \`- \`；发布转换时有序列表会保留编号，无序列表会转成 \`•\`。

首屏 Hook 规则：
- 导语严格控制在 120 字以内，必须命中「场景 / 痛点 / 损失 / 收益承诺 / 反差对比」中至少 2 个元素。
...
5. 最后一行单独一段来源说明，格式：\`来源：<YouTube 或原页面 URL>\`（若上游未提供 URL，用占位 \`来源：（见 metadata）\` 并在同一行附视频标题）。
