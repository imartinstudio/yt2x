...
```text
Markdown 预处理 → HTML + block_index 元数据 → Browser Clipboard API → 浏览器操作 X Articles 编辑器 → 仅保存草稿
```
...
- 依赖：本机已登录 X Premium、Playwright、浏览器 Clipboard API
...
| 浏览器 Clipboard API | 富文本 / 正文图片粘贴   | Playwright context 授予 x.com clipboard 权限 |
...
- 方案 A：`packages/adapters-node` 子进程调用 vendored / 文档化的 `parse_markdown.py`（从 publish-x-article 移植并加 LICENSE 注明）。
...
- [x] Task 6 implementation landed
- [ ] Task 6 complete
...
- [x] Task 9 implementation landed
- [ ] Task 9 complete
...
- [x] Task 13 unit and package verification complete
- [ ] Task 13 complete
...
- 当前 MVP 采用 TypeScript 解析与 Browser Clipboard API，没有 vendored Skill 脚本。
...
- [x] `pnpm yt2x publish --video-id <videoId> --target article --dry-run` 只写 `publish-preview.json`，不启浏览器、不调 API。（单测覆盖）
- [x] `pnpm yt2x publish --video-id <videoId> --target article`（无 flag）明确失败并提示 `--browser-draft`。（单测覆盖）
- [ ] `pnpm yt2x publish --video-id <videoId> --target article --browser-draft` 在本机 Premium 账号下将长文写入 X 草稿箱。
- [x] `publish-result.json` 含 `mode: "article-draft"` 与时间戳。（adapter mock 单测覆盖）
- [x] `process-status.json` 的 `publish` 步与 API 发布共用契约、可区分 `mode`。（adapter mock 单测覆盖）
- [x] `pnpm yt2x publish --target x-thread` 回归通过。（单测覆盖）
- [x] `pnpm run ci` 通过。
...
| Playwright 浏览器依赖重 | opt-in flag；依赖检查与文档；默认发布预览不启浏览器                |
