...
- 如果单条 tweet 内部需要列步骤或要点，序号后加一个空格，内容紧跟在同一行：使用纯数字序号 \`1\`、\`2\`、\`3\`（不加点、不加括号、不加斜杠），格式为 \`1 内容\`、\`2 内容\`；也允许 \`① 内容\`、\`1️⃣ 内容\` 等圈号和 emoji 数字。不要写成 \`1/ 内容\`（串推编号）或把序号单独放一行。
...
- 禁止在 tweets 中使用 Markdown 表格或竖线分列表格，例如 \`| A | B |\`、\`| --- | --- |\`；如需表达对比、参数或步骤，改写成纯文本短行，并遵守冒号后换行、序号后加空格规则。
- ${SHARED_NO_CLICKBAIT}
- ${SHARED_NO_VIDEO_AUTHOR}
- ${SHARED_LANG_ZH_CN}
