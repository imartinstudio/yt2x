...
封面与配图规范：
- 文章开头必须有一张封面图，使用 \`![封面](images/cover.png)\` 单独成段放在标题下方。
- 封面比例 5:2（1500×600），适合 X 信息流卡片预览。封面应概括文章核心主题，一个强有力的视觉隐喻。
- 正文插图比例 16:9，适合移动端横屏查看。插图用于解释具体界面、流程、对比或验证结果。
- 封面和插图描述需包含在输出的 JSON 的 \`cover\` 和 \`illustrations\` 字段中，含 \`visual_prompt\`（供后续 AI 图片生成使用）。
