...
- CLI 参数名、代码标识符、用户可见输出（日志、帮助文本、画面上的文字）三者必须使用
  [CONTEXT.md](./CONTEXT.md) 里的同一个词。
- 新增领域概念时，先在 `CONTEXT.md` 定义，再写代码；不要先造标识符再回头补词汇。
- 改动会渲染到用户眼前的东西时（画面文字、帮助文本），检查承载它的标识符是否还叫得对。

这条规则的代价是实测过的：曾有一处水印在画面上印「字幕：」，代码里却叫 `watermarkXlate`
（PR #147 修复）。后果不是不整洁——按代码里的名字检索找不到该功能，导致得出「主干没有水印
功能」的错误结论。词表检查拦不住这类漂移（两个名字都是合法的词），只有 review 时逐个问
「这个名字对得上它渲染出来的东西吗」才拦得住。

## CLI 参数归属

同一个能力有多个入口时，参数放在哪一层按下面的判据划线：

- **影响交付物长什么样**的参数 → 放在交付入口（`yt2x video` / `yt2x text`）。
- **控制跑到哪一步、或指定外部程序路径**的参数 → 只放在诊断/单步命令，不上浮。

例：`--dub-engine` / `--voice` / `--original-voice-volume` 决定成片听起来如何，属交付入口；
`--script-only` / `--timing-only` / `--start-ms` / `--ffmpeg-path` 只控制流程或环境，留在
`yt2x dub`。

另外两条硬约束：

- 不要为「运行环境里某个程序在哪」设计用户参数。同一个 Python 解释器可能满足 Pillow、
  faster-whisper、demucs 中的一部分而非全部，让用户传一个路径去覆盖多种能力必然出错。
  按能力探测（见 `packages/adapters-node/src/acquire/resolve-python.ts`），不要新增
  `--*-path` 类参数。
- 不要静默改写用户的显式输入。互相矛盾的显式参数一律报错并说明原因；未指定时才使用默认值，
  且默认值的选用要记录到日志。
