...
export const registerPublishCommand = (program: Command): void => {
  ...
  addCommonSourceOptions(cmd)
    ...
    .option("--browser-draft", "Save --target article into X Articles drafts through a logged-in browser", false)
    .option("--x-subscription <tier>", "X Articles tier: premium|premium-plus", "premium")
    .option("--browser-profile-dir <path>", "Persistent browser profile for X Articles login")
    .option("--headless", "Run browser-draft automation headless", false)
