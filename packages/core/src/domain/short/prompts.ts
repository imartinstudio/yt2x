...
写作规则：
- 只生成 1 条短帖正文。
- 短帖建议控制在 18 行以内（理想 14–18 行），必须精炼表达核心判断；不要堆砌细节，不要把长文压缩成流水账。
- Post 文本格式规则必须和串推保持一致：text 中不要使用 Markdown 加粗、行内代码、代码块、有序列表、无序列表、Markdown 链接、引用或表格；只使用普通纯文本、自然换行和原始 URL。
- 可以在单条短帖内部使用纯文本序号组织 4–6 个总结要点。序号后加一个空格，内容紧跟在同一行：使用纯数字序号 \`1\`、\`2\`、\`3\`（不加点、不加括号、不加斜杠），格式为 \`1 内容\`、\`2 内容\`；也允许 \`① 内容\`、\`1️⃣ 内容\` 等圈号和 emoji 数字。不要写成 \`1/ 内容\`（串推编号）或把序号单独放一行。
- 不要输出多个备选版本。
...
- 禁止在短帖中使用 Markdown 表格或竖线分列表格，例如 \`| A | B |\`、\`| --- | --- |\`；如需表达对比、参数或步骤，改写成纯文本短行，并遵守冒号后换行、序号后加空格规则。
- ${SHARED_NO_CLICKBAIT}
- ${SHARED_NO_VIDEO_AUTHOR}
- ${SHARED_LANG_ZH_CN}
...
- 短帖里的要点必须规范：使用纯数字序号 \`1\`、\`2\`、\`3\` 后加空格，内容紧跟在同一行；不要混用串推编号 \`1/\`、\`2/\`，不要使用 Markdown list marker。
- list item 如果是「标题：正文」结构，必须在冒号后换行；每个 list item 之间只换行，不额外空一行。
- 当正文句子中通过括号序号（\`1）\`、\`2）\`、\`3）\`）或点号序号（\`1.\`、\`2.\`、\`3.\`）连续列举多项内容时，每个序号前必须换行，每个序号项单独成行。例如「核心方法：1）A；2）B；3）C」必须拆分为每项一行：先写「核心方法：」然后换行，再依次写「1）A；」换行、「2）B；」换行、「3）C」。

可执行资产规则：
- 4–6 条 list item 中，至少有 1 条必须给出**具体可复制的例子**（而非只提概念），例如：一个完整的 3–5 步 SOP 示例、一句可直接使用的 prompt、一份最小检查清单。
...
- 推荐结构：第一段一句话强判断（必须有明显冲突或反差）；第二段引出内容核心；中间 4–6 条总结 list（使用「短标题 + 空行」格式，每个 list item 之间空一行，含至少 1 条具体可执行例子，必要时含 1 条风险提醒）；最后一句明确读者收益和具体下一步动作。
- 使用纯数字序号 \`1\`、\`2\`、\`3\` 后加空格，内容紧跟在同一行；每个 list item 之间换行分隔。

输出语言硬约束：全文必须使用简体中文（zh-CN）。禁止繁体中文。这是硬性要求，不可违反。
