...
## 规则
- **只输出有对应视频画面的章节**。如果某个观点只在文章中有、在字幕/视频中找不到对应片段，不要输出。durationSec 必须大于 0 且不超过 120 秒（2 分钟）。超过 120 秒的片段说明拆得太粗，需要进一步拆分。
...
- 所有输出的中文内容（title、summary、article_section、key_quote、video_script）必须使用简体中文（zh-CN）。禁止繁体中文。原始材料中的繁体中文必须转写为简体。这是硬性要求。
...
export const buildDeconstructUserPrompt = (
  ...
  parts.push("请识别所有独立章节（每个片段 ≤ 120 秒），匹配时间码（使用索引中已有时间戳即可），进行多维度评分，输出严格 JSON。");
  return parts.join("\n");
};
